...
import { buildCycleFromAnnual } from '../planningStageChain.js';
import {
  createCycle90Data,
  cycle90CoverFields,
  cycle90Progress,
...
export default function Cycle90Plan() {
  ...
  return (
    <PlanningDocumentEditor
      ...
      previousStage={CYCLE_PREVIOUS_STAGE}
    />
  );
}
...
const CYCLE_PREVIOUS_STAGE = {
  sourceType: 'annual_plan',
  sourceLabel: 'Plano de Ação Anual',
  async loadSource(context) {
    const { data } = await api.get('/action-plans', {
      params: { client_id: context.clientId, year: context.year },
    });
    const annual = data.plan?.annual_plan_data || data.plan?.annual_plan || null;
    if (!hasPlanningContent(annual)) return null;
    const quarter = Number(String(context.periodKey).split('Q')[1]) || 1;
    const startMonth = (quarter - 1) * 3;
    return {
      data: annual,
      updatedAt: data.plan?.updated_at || null,
      periodKey: String(context.year),
      periodLabel: `Plano anual · ${context.year}`,
      context: {
        quarter,
        quarterLabel: quarterTheme(quarter),
        startDate: toDateInput(new Date(context.year, startMonth, 1)),
        endDate: toDateInput(new Date(context.year, startMonth + 3, 0)),
      },
    };
  },
  buildData(current, source, context, mode) {
    return buildCycleFromAnnual(current, source, context, mode);
  },
};
