...
router.put('/:id', requireRole('admin', 'team'), (req, res) => {
  const { name, segment, logo_color, status, avatar_data, avatar_mime, bio } = req.body;
  db.prepare(
    'UPDATE clients SET name = COALESCE(?, name), segment = COALESCE(?, segment), logo_color = COALESCE(?, logo_color), status = COALESCE(?, status), avatar_data = COALESCE(?, avatar_data), avatar_mime = COALESCE(?, avatar_mime), bio = COALESCE(?, bio) WHERE id = ?'
  ).run(name, segment, logo_color, status, avatar_data, avatar_mime, bio, req.params.id);
  res.json({ ok: true });
});
